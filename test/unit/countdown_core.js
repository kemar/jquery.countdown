QUnit.test("Markup structure", function (assert) {
    ...
    assert.equal(timeElement.find('.item').length, 4, ".item elements exist, correct number");
    assert.equal(timeElement.find('.separator').length, 3, ".separator elements exist, correct number");

    // Days infos should be hidden.
    ...
    assert.equal(timeElement.find('.separator').eq(0).text(), ',', "Default separator for days");
    assert.equal(timeElement.find('.separator').eq(1).text(), ':', "Default separator for hours");
    assert.equal(timeElement.find('.separator').eq(2).text(), ':', "Default separator for minutes");

    assert.equal(timeElement.find('.label-dd').text(), 'days', "Default days label");
